...
create policy if not exists "Anyone can view open matches"
  on public.matches
  for select
  using (status = 'open');

-- Players can view their own matches
create policy if not exists "Players can view their matches"
  on public.matches
...
create policy if not exists "Users can view their balance"
  on public.balances
  for select to authenticated
  using (user_id = auth.uid());

create policy if not exists "Users can create their balance"
  on public.balances
  for insert to authenticated
  with check (user_id = auth.uid());

create policy if not exists "Users can update their balance"
  on public.balances
  for update to authenticated
  using (user_id = auth.uid());

create trigger if not exists update_balances_updated_at
...
create policy if not exists "Users can create their transactions"
  on public.transactions
  for insert to authenticated
  with check (user_id = auth.uid());
